...
def read_tiktok_returns(file_like) -> pd.DataFrame:
    """
    TikTok return export. Often arrives with only a 'Return Order ID' header and
    no data rows -- callers should check len(df) == 0 and surface that as a warning
    rather than silently skipping it.

    Note: TikTok's order-id header is 'Return Order ID', and the actual TikTok
    order identifier shared with TC/other systems is in a separate 'Order ID'
    column when present. We standardize to a column literally named 'Order ID'
    for downstream matching, without dropping any of the other columns (return
    reason, SKU, tracking, etc. all live in this same dataframe).
    """
    df = _read_excel_any(file_like)
    df.columns = [str(c).strip() for c in df.columns]

    # Prefer an actual 'Order ID' column if TikTok provided one distinct from
    # 'Return Order ID'; otherwise fall back to whichever column has "order id"
    # in its name.
    ...
    df["Order ID"] = df[id_col].apply(normalize_order_id)
    return df


def _read_excel_any(file_like) -> pd.DataFrame:
    """
    Read an Excel file regardless of whether it's legacy .xls or modern .xlsx.
    Accepts a path, bytes, or a file-like object (e.g. Streamlit's UploadedFile).
    """
    name = getattr(file_like, "name", "") or ""
    raw = file_like.read() if hasattr(file_like, "read") else None
    if raw is not None and hasattr(file_like, "seek"):
        file_like.seek(0)

    buf = io.BytesIO(raw) if raw is not None else file_like
    ...
    try:
        ...
        # Some "modern" exports are actually legacy-format under a misleading
        # extension; fall back to xlrd as a last resort.
        if raw is not None:
            buf2 = io.BytesIO(raw)
            return pd.read_excel(buf2, engine="xlrd")
        raise
